...
pub fn part_one(input: &str) -> Option<u32> {
    Some(input.trim().lines().map(|line| {
        let chars = line.chars().into_iter();
        
        let first = line.chars().into_iter().find(|c| c.is_digit(10))
            .expect("Numeric Char always exist");

        let last = line.chars().into_iter().rev().find(|c| c.is_digit(10))
            .expect("Numeric Char always exist");
...
pub fn part_two(input: &str) -> Option<u32> {
    ...
    Some(input.trim().lines().map(|line| {
        ...
        let first = numbers.clone().min_by_key(|i| i.0).expect("At least one match").1;

        let last = numbers.clone().max_by_key(|i| i.0).expect("At least one match").1;

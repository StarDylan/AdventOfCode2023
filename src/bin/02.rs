...
impl Game {
    fn from_str(s: &str) -> Self {
        ...
        let game = parts
            .next().expect("Always a game label")
            .split_ascii_whitespace().skip(1).next()
            .expect("Always a game id")
            .parse().expect("Always an integer");

        let rounds = parts.next().expect("Always rounds").trim()
            .split("; ");
        
        let rounds = rounds.map(|round| {
            let mut red = 0;
            let mut green = 0;
            let mut blue = 0;
            ...
            round.split(", ").for_each(|cube_combo| {
                ...
                let num: u32 = round_parts.next()
                    .expect("Always a number")
                    .parse().expect("Always integer");
...
pub fn part_one(input: &str) -> Option<u32> {
    ...
    Some(games.map(|game| {
        let possible = game.rounds.iter()
            .find(|round| round.red > 12 || round.green > 13 || round.blue > 14).is_some();
...
pub fn part_two(input: &str) -> Option<u32> {
    ...
    Some(games.map(|game| {
        let min_game_config = game.rounds.iter()
            .fold(Round::zeros(),|min_cubes, round|{
                Round { 

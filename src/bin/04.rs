...
impl Card {
    fn from_str(s: &str) -> Card {
        ...
        let caps = RE.captures(s)
            .expect(&format!("{s} matches regex"));
        
        Card {
            card_number: caps["card_num"].parse().expect("Valid u32"),
            winning_nums: HashSet::from_iter(caps["winning_nums"]
                .split_ascii_whitespace()
                .map(|num| num.parse().expect("Valid u32 winning nums"))),

            numbers_we_have: HashSet::from_iter(caps["our_nums"]
                .split_ascii_whitespace()
...
pub fn part_one(input: &str) -> Option<u32> {
    Some(input.lines().map(Card::from_str)
        .map(|card| {
            let winning_nums = card.numbers_we_have.intersection(&card.winning_nums).count() as u32;
...
fn process_scratch_card(cards: &Vec<Card>, scratch_card_to_process: usize) -> u32 {
    let card_to_process = cards.get(scratch_card_to_process - 1)
        .expect("Valid input to function");

    let wins = card_to_process.numbers_we_have.intersection(&card_to_process.winning_nums).count();

    if wins == 0 {
        return 1;
    } else {
        // Get next scratch cards
        cards.iter().enumerate().skip(scratch_card_to_process).take(wins)
            .map(|(idx, card)| process_scratch_card(cards, idx + 1))
            .sum::<u32>() + 1
    }
